...
FloatN Math::fract(const FloatN x, FloatNPtr iptr) noexcept
{
  static_assert(kIsFloatingPoint<FloatN>, "The FloatN isn't floating point.");
  ...
  const auto y = Config::isBuiltinMathFractUsed()
    ? Builtin::fract(x, iptr)
    : Zinvul::fract(x, iptr);
  return y;
}
...
/*!
  */
template <typename FloatN, typename FloatNPtr> inline
FloatN Math::Builtin::fract(const FloatN x, FloatNPtr iptr) noexcept
{
  static_assert(kIsFloatingPoint<FloatN>, "The FloatN isn't floating point.");
  using ASpaceInfo = AddressSpaceInfo<FloatNPtr>;
  using FloatNType = typename ASpaceInfo::DataType;
  static_assert(kIsFloatingPoint<FloatNType> & ASpaceInfo::isPointer(),
                "The FloatNPtr isn't floating point.");
//  const auto y = ZINVUL_GLOBAL_NAMESPACE::fract(x, iptr);
  const auto y = make<FloatN>(0.0f);
  return y;
}
...
/*!
  */
template <typename FloatN, typename FloatNPtr> inline
FloatN Math::Zinvul::fract(const FloatN x, FloatNPtr iptr) noexcept
{
  static_assert(kIsFloatingPoint<FloatN>, "The FloatN isn't floating point.");
  using ASpaceInfo = AddressSpaceInfo<FloatNPtr>;
  using FloatNType = typename ASpaceInfo::DataType;
...
FloatN Math::Zinvul::frexp(const FloatN x, IntegerNPtr e) noexcept
{
  ...
  static_assert(kIsInteger<IntegerN> & ASpaceInfo::isPointer(),
                "The IntegerNPtr isn't integer pointer type.");
  using FloatType = typename VectorTypeInfo<FloatN>::ElementType;
  using FloatInfo = FloatingPointFromBytes<sizeof(FloatType)>;
  using BitVec = UIntegerTypeFromVec<FloatN>;

  constexpr auto exp_mask = FloatInfo::exponentBitMask();
  const auto zero = make<FloatN>(static_cast<FloatType>(0.0));
  const auto is_special = isequal(zero, x) | isinf(x) | isnan(x);
