...
TEST(VectorDataTest, Int8bVectorDataLocalTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testInt8bVectorDataLocal));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, Int8bVectorDataPrivateTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testInt8bVectorDataPrivate));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, Int16bVectorDataLocalTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testInt16bVectorDataLocal));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, Int16bVectorDataPrivateTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testInt16bVectorDataPrivate));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, Int32bVectorDataLocalTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testInt32bVectorDataLocal));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, Int32bVectorDataPrivateTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testInt32bVectorDataPrivate));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, UInt32bVectorDataLocalTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testUInt32bVectorDataLocal));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, UInt32bVectorDataPrivateTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testUInt32bVectorDataPrivate));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, FloatVectorDataLocalTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testFloatVectorDataLocal));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, FloatVectorDataPrivateTest)
{
  using namespace zinvul;
  ...
  for (std::size_t number = 0; number < device_list.size(); ++number) {
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testFloatVectorDataPrivate));
    kernel->run(*buffer1, *buffer2, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, HalfUVectorDataLocalTest)
{
  using namespace zinvul;
...
//                    << ", f = " << std::scientific << t << std::endl;
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testHalfUVectorDataLocal));
    kernel->run(*(buffer1->treatAs<uint16b>()), *buffer2, *buffer3, *res_buff, {resolution}, 0);
    device->waitForCompletion();
...
TEST(VectorDataTest, HalfUVectorDataPrivateTest)
{
  using namespace zinvul;
...
//                    << ", f = " << std::scientific << t << std::endl;
    ...
    auto kernel = makeKernel<1>(device.get(), ZINVUL_MAKE_KERNEL_ARGS(vector_data, testHalfUVectorDataPrivate));
    kernel->run(*(buffer1->treatAs<uint16b>()), *buffer2, *buffer3, *res_buff, {resolution}, 0);
    device->waitForCompletion();
