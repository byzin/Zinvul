...
uint32b atomicInc(__global uint32b* p)
{
  uint32b old = *p;
  uint32b expected_old = 0;
  do {
    expected_old = old;
    const uint32b v = expected_old + 1;
    old = atomic_cmpxchg(p, expected_old, v);
  } while (old != expected_old);
  return old;
}
...
kernel void testAtomicCmpxchgUint(global uint32b* result, global int32b* table, const uint32b resolution)
{
  const uint32b index = zGetGlobalIdX();
  if (index < resolution) {
    const uint32b i = atomicInc(result);
    table[i] = 1;
  }
}
